<div class="w-80 p-4 bg-white shadow-lg rounded-lg h-full relative">
    ...
    @else
        <div class="mb-4 max-h-124 overflow-y-auto soft-scrollbar"
             ...
             @scroll.debounce.150ms="if ($el.scrollTop + $el.clientHeight >= $el.scrollHeight - 10 && !isLoading) {
                isLoading = true;
                page++;
                $wire.loadMore().then(() => isLoading = false);
             }">

            <!-- Use wire:ignore for just the scroll container but keep the children interactive -->
