...
<div class="max-w-sm mx-auto">
    ...
    <form wire:submit="login">
        <div class="flex flex-col gap-4">
            ...
            <x-primary-button wireTarget='login' onclick="openOtherLogin()">
                Login
            </x-primary-button>
        ...
        <div class="flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
            @if (Route::has('password.request'))
                <a class="text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    href="{{ route('password.request') }}" wire:navigate onclick="openOtherLogin()">
                    {{ __('Forgot your password?') }}
                </a>
            @endif
            @if (Route::has('register'))
                <a class="underline text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    href="{{ route('register') }}" wire:navigate onclick="openOtherLogin()">
        ...
        <div class="flex flex-col items-center gap-2">
            ...
            {{-- <a id="google-login-btn" href="javascript:void(0);" class="flex items-center justify-center w-full py-2 px-4 border border-gray-300 text-gray-700 text-base font-bold rounded hover:border-gray-400" onclick="openGoogleLoginPopup()">
                ...
                {{ __('Login with Google') }}
                    ...
                    <!-- Google Login Button -->
            <a href="{{ route('google.login') }}" class="flex items-center justify-center w-full py-2 px-4 border border-gray-300 text-gray-700 text-base font-bold rounded hover:border-gray-400">
                <i class="fab fa-google m-2"></i> Sign in with Google
            </a>

            <!-- Facebook Login Button -->
            {{-- <a id="facebook-login-btn" href="{{ route('facebook.login') }}" class="flex items-center justify-center w-full py-2 px-4 border border-gray-300 text-gray-700 text-base font-bold rounded hover:border-gray-400" onclick="openOtherLogin()">
...
// GOOGLE
// Function to open the Google login popup in the center of the screen
function openGoogleLoginPopup() {
    const googleLoginUrl = "{{ route('google.login') }}";

    const popupWidth = 500;  // Width of the popup window
